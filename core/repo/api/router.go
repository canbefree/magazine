...
type CrudAPI struct {
	...
	TableInfo       *repo_model.TableInfo `json:"table_info"`
}
...
func ConfigRouter() http.Handler {
	...
	configMInventoryRouter(router)
	configMInventoryDecrRouter(router)
	configMInventoryIncrRouter(router)
	configMInventoryTranRouter(router)
...
func ConfigGinRouter(router gin.IRoutes) {
	configGinMInventoryRouter(router)
	configGinMInventoryDecrRouter(router)
	configGinMInventoryIncrRouter(router)
	configGinMInventoryTranRouter(router)
...
func ValidateRequest(ctx context.Context, r *http.Request, table string, action repo_model.Action) error {
...
type RequestValidatorFunc func(ctx context.Context, r *http.Request, table string, action repo_model.Action) error
...
func returnError(ctx context.Context, w http.ResponseWriter, r *http.Request, err error) {
	status := 0
	switch err {
	case repo_dao.ErrNotFound:
		status = http.StatusBadRequest
	case repo_dao.ErrUnableToMarshalJSON:
		status = http.StatusBadRequest
	case repo_dao.ErrUpdateFailed:
		status = http.StatusBadRequest
	case repo_dao.ErrInsertFailed:
		status = http.StatusBadRequest
	case repo_dao.ErrDeleteFailed:
		status = http.StatusBadRequest
	case repo_dao.ErrBadParams:
		status = http.StatusBadRequest
	default:
		status = http.StatusBadRequest
	}
...
// GetDdl is a function to get table info for a table in the magazine database
// @Summary Get table info for a table in the magazine database by argID
// @Tags TableInfo
// @ID argID
// @Description GetDdl is a function to get table info for a table in the magazine database
// @Accept  json
// @Produce  json
...
func GetDdl(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if err := ValidateRequest(ctx, r, "ddl", repo_model.FetchDDL); err != nil {
...
// GetDdlEndpoints is a function to get a list of ddl endpoints available for tables in the magazine database
// @Summary Gets a list of ddl endpoints available for tables in the magazine database
// @Tags TableInfo
// @Description GetDdlEndpoints is a function to get a list of ddl endpoints available for tables in the magazine database
// @Accept  json
// @Produce  json
// @Success 200 {object} api.CrudAPI
// @Router /ddl [get]
...
func init() {
	...
	tmp = &CrudAPI{
		Name:            "m_inventory",
		CreateURL:       "/minventory",
		RetrieveOneURL:  "/minventory",
		RetrieveManyURL: "/minventory",
		UpdateURL:       "/minventory",
		DeleteURL:       "/minventory",
		FetchDDLURL:     "/ddl/m_inventory",
	}

	tmp.TableInfo, _ = repo_model.GetTableInfo("m_inventory")
	crudEndpoints["m_inventory"] = tmp

	tmp = &CrudAPI{
		Name:            "m_inventory_decr",
		CreateURL:       "/minventorydecr",
		RetrieveOneURL:  "/minventorydecr",
		RetrieveManyURL: "/minventorydecr",
		UpdateURL:       "/minventorydecr",
		DeleteURL:       "/minventorydecr",
		FetchDDLURL:     "/ddl/m_inventory_decr",
	}

	tmp.TableInfo, _ = repo_model.GetTableInfo("m_inventory_decr")
	crudEndpoints["m_inventory_decr"] = tmp

	tmp = &CrudAPI{
		Name:            "m_inventory_incr",
		CreateURL:       "/minventoryincr",
		RetrieveOneURL:  "/minventoryincr",
		RetrieveManyURL: "/minventoryincr",
		UpdateURL:       "/minventoryincr",
		DeleteURL:       "/minventoryincr",
		FetchDDLURL:     "/ddl/m_inventory_incr",
	}

	tmp.TableInfo, _ = repo_model.GetTableInfo("m_inventory_incr")
	crudEndpoints["m_inventory_incr"] = tmp

	tmp = &CrudAPI{
		Name:            "m_inventory_tran",
		CreateURL:       "/minventorytran",
		RetrieveOneURL:  "/minventorytran",
		RetrieveManyURL: "/minventorytran",
		UpdateURL:       "/minventorytran",
		DeleteURL:       "/minventorytran",
		FetchDDLURL:     "/ddl/m_inventory_tran",
	}

	tmp.TableInfo, _ = repo_model.GetTableInfo("m_inventory_tran")
	crudEndpoints["m_inventory_tran"] = tmp
